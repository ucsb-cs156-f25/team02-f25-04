...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three organizations correctly for regular user", async () => {
    ...
    await waitFor(() => {
      ...
      ).toHaveTextContent("CYB");
    });
    expect(
      screen.getByTestId(`${testId}-cell-row-1-col-orgCode`),
    ).toHaveTextContent("PSY");
    expect(
      screen.getByTestId(`${testId}-cell-row-2-col-orgCode`),
    ).toHaveTextContent("CHE");

    const createButton = screen.queryByText("Create Organization");
    expect(createButton).not.toBeInTheDocument();

    const orgTranslationShort = screen.getByText("Cybersecurity");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Cybersecurity-club");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with orgCode CYB was deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(
        "UCSBOrganization with orgCode CYB was deleted",
      );
    });
    ...
    expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "CYB" });
  });
});
